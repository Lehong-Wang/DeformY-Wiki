...
\State Execute DEFORM \Comment{Algorithm. \ref{alg:long time horizon}, Assumption.\ref{assum: static initial}}
\EndWhile
...
Fortunately, we can address this problem by making the following assumption:
\begin{assum}
When the sensor measurements begin, the DLO is static and is only subject to gravity and/or the manipulators which are holding its ends.
% when t=1, DLO is static and is only subjected to force from gravity and grippers.
\label{assum: static initial}
\end{assum}
...
% \begin{algorithm}
% \caption{Perception with Modeling} 
% \label{alg:inextensibility of PBD}
% \textbf{Initial Perception}: $\textbf{S}_1$
% \begin{algorithmic}[1]
% \State $\Tilde{\textbf{S}}_1\in \{||\hat{\textbf{X}}_1 - \textbf{S}_1||_2 \leq threshold\}$ \Comment{\secref{sec:initial_state}}
% \State Obtain $\textbf{X}_1$
% \Comment{\secref{sec:initial_state}, \secref{sec: AdaptiveGMM}}
% \Repeat
%     \For{$t = 1$ \textbf{to} $T-1$}
%         \State Execute \textbf{Algorithm. \ref{alg:long time horizon}}
%     \EndFor
%     \State \textbf{Perception:} $\textbf{S}_T$, Obtain $\textbf{X}_T$ \Comment{\secref{section: filter} \ref{sec: AdaptiveGMM}}
% \Until{Stop}
% \end{algorithmic}
% \end{algorithm}